...
def partial_subsequence_match(pattern, text):              #string matching
    lcs = longest_common_subsequence(pattern, text)
    return len(lcs) == min(len(pattern),len(text))


def get_google_search_results(id,website):                #get url from google search
    query = ""
    query+=id+" "+website
...
def get_img_src(url):                                 #get image url
        try:
...
def product_id_vijaysales(url):                        #get product_id from vijaysales
    product_id = None
    try:
        r = requests.get(url)
...
def product_id_croma(url):                             #get product_id from croma
    product_id = None
    try:
        r = requests.get(url)
        soup = BeautifulSoup(r.content, 'html.parser')
...
def product_id_reliance(url):                                #get product_id from reliance
    product_id = None
    try:
        r = requests.get(url)
        soup = BeautifulSoup(r.content, 'html.parser')
...
def product_id_flipkart(url):                             #get product_id from flipkart
    product_id = None
    try:
        r = requests.get(url)
        soup = BeautifulSoup(r.content, 'html.parser')
...
def populate_data(product_id,website):                         #populate data using webscraping into the database
...
def get_product_id(URL):                                     #get product_id from the base url
    product_id = None
    website = None
    r = requests.get(URL)
    soup = BeautifulSoup(r.content, 'html.parser') 
